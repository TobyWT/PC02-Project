...
class Graph:
    ...
    def solve_dijkstra(self, later_list: [Node], end: Node, canvas: tkinter.Canvas,
        ...
        if current == end:
            final_path = [end]
            path_tracker = end
            while path_tracker.parent is None:
        ...
        for col_index in range(len(self.nodes)):
            if self.adjacency_matrix[current_node_index][col_index] == 0:
    ...
    def draw_path(self, canvas, scale_size):
        ...
        for node_counter in range(len(self.found_path) - 1):
            current_node = self.found_path[node_counter]
            next_node = self.found_path[node_counter + 1]
            canvas.create_line((current_node.x + 0.5) * scale_size * 3, (current_node.y + 0.5) * scale_size * 3,
                               (next_node.x + 0.5) * scale_size * 3, (next_node.y + 0.5) * scale_size * 3,
                               fill="#FF0000", width=3)


class MazeSolvingProgram:
    def __init__(self, maze_size, scale_size):
        # Prepare the maze and the tkinter GUI elements
        ...
        self.__dijkstra_canvas.after(1000, lambda: self.solve(self.__dijkstra_graph, self.__dijkstra_canvas, dijkstra_square_grid))
    ...
    def solve(self, maze: Graph, canvas: tkinter.Canvas, node_square_grid) -> None:
